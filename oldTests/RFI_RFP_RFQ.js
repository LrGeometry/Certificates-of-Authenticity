...
it('returns rfp metadata correctly',async function(){
    ...
    let rfp=await RFPinstance.getRFP(2,{from:accounts[1]})
   //console.log(rfp)
 ...
 it('returns rfq metadata correctly',async function(){
    ...
    let rfq=await RFQinstance.getRFQ(3,{from:accounts[1]})
...
it('changes RFQ ES data',async function(){
    await RFQinstance.changeEquivalentSubstitution(3,'some changed data(*(()',{from:accounts[1]})
    let rfq=await RFQinstance.getRFQ(3,{from:accounts[1]})
...
it('changes RFP ES data',async function(){
    await RFPinstance.changeEquivalentSubstitution(2,'some changed data(*(()',{from:accounts[1]})
    let rfp=await RFPinstance.getRFP(2,{from:accounts[1]})
