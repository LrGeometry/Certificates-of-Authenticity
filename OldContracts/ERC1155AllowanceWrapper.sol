...
contract AllowanceWrapper is IERC1155, ERC165
{
    ...
    */
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) public /*payable*/  {
        if (msg.sender != _from) {
    ...
    /**
        ...
        @param _data    Additional data with no specified format, sent in call to `_to`
    */
    function safeBatchTransferFrom(address _from, address _to, uint256[] calldata _ids, uint256[] calldata _values, bytes calldata _data) public /*payable*/ {
        if (msg.sender != _from) {
