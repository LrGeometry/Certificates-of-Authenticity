...
contract("tests the NFT creator",(accounts)=>{
    ...
    it('Create initial items and adds them to list', async () => {
       ...
       let tx = await MetaERC.create(Quantity, Uri,"Herculus2","HERC2", {from: user1});
        tx = await MetaERC.create(Quantity, Uri,"Herculus3","HERC3", {from: user1});
       // tx = await MetaERC.createfor(Quantity,Uri, Uri2,user1,"Herculus2","HERC2",0);
        
        let list=await  MetaERC.getAllOwnedTokens(user1)
        assert.equal(list[0].toNumber(),2,"token 2 is in list")
        assert.equal(list[1].toNumber(),3,"token 3 is in list")
